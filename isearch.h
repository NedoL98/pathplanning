...
class ISearch
{
    ...
    protected:
        ...
        double computeHFromCellToCell(const Node &from, const Node &to, const EnvironmentOptions &options);
        ...
        std::vector<int> getDifference(Node &curNode, Node &nextNode);
        bool check(const Node &curNode, std::vector<int> dm, const Map &map, const EnvironmentOptions &options);
        bool compareByCoordinate(const Node &node1, const Node &node2);
